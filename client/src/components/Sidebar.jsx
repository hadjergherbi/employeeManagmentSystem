...
const handleLogout = ()=>{
  ...
  const sidebarContent = (
    <>
      ...
      {/* User profile card */}
      {userName && (
        <div className="mx-3 mt-4 mb-1 p-3 rounded-lg bg-white/3 border border-white/40">
            <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                    ...
                    <p className="text-[11px] tetx-slate-500 truncate"> {role === "ADMIN" ? "Administrator" : "Employee"}</p>
                </div>
      ...
      <div>
        <div className="flex-1 px-3 space-y-0.5 overflow-y-auto">
           {navItems.map((item)=>{
            ...
            return(
                <Link key={item.name} to={item.href} className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 relaative ${isActive ? "bg-indigo-500/12 text-indigo-300" : "hover:text-white hover:bg-white/4"}`}>
